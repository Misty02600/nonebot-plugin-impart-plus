...
- 挑战状态由当前持有阶级及下一阶区间推导：一阶25→30、二阶300→320、三阶1000→1050。进入时分别将当前胜率乘0.9/0.8/0.7一次，退出分别乘10/9、1.25、10/7恢复；每次只应用本次挑战系数。正负各阶试炼名统一为“登神长阶／深渊试炼”。挑战者禁止自身成长，也不能主动嗦或舔任何世界的目标；作为目标时同样禁止被嗦或被舔。发起者的拒绝回复按当前指令填入动作，拒绝不消耗冷却和成长随机数。已解锁的夺舍仍可使用。
