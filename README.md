...
`nonebot_plugin_impart_plus` 是面向 NoneBot2 群聊场景的互动插件，围绕群内数值成长、
PK、状态挑战和群友互动提供完整玩法。当前发布声明仍为 OneBot V11；接入层已经使用
Alconna、Uninfo 与 UniMessage，为后续扩大 Adapter 支持保留统一边界。
...
命令允许省略子命令前的空格，例如 `银趴 开启` 与 `银趴开启` 等价。

| 指令 | 权限 | 目标 | 说明 |
|:--|:--|:--|:--|
| `银趴开启`、`银趴禁止` | 管理员、群主或超级用户 | 当前群 | 开启或关闭群内玩法；`开始/关闭`是子命令 alias |
| `银趴帮助` | 群成员 | 当前群 | 输出插件命令说明；`介绍`是子命令 alias |
| `透/日` + `群友/群主/管理` | 群成员 | 群友、群主或管理 | 随机选择或通过 `@` 指定群友，如 `透群友`、`日 管理` |
| `pk`、`对决` | 群成员 | 需要 `@` 对手 | 根据双方战力结算长度变化 |
| `打胶`、`开导` | 群成员 | 自己 | 增加自己的长度 |
| `嗦牛子`、`嗦`、`suo` | 群成员 | 自己或 `@` 用户 | 增加目标长度 |
| `查询` | 群成员 | 自己或 `@` 用户 | 查询长度和当前状态 |
| `银趴排行榜`、`银趴排名`、`银趴榜单`、`银趴rank` | 群成员 | 全部用户 | `impart` 可替代 `银趴`；显示前五、后五及自己的排名 |
